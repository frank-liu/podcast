...
### Primary User Story
As a user, I want to receive a daily audio digest summarizing the top 10 messages (short news items)
for a chosen topic, in either English or Chinese, so I can consume the latest news hands-free.

### Acceptance Scenarios
1. Given a registered user with a selected topic and language, when the digest job runs for the day,
   then the system produces a single audio file summarizing up to 10 messages and publishes it to the
   user's connected Spotify account, and the user receives a notification with the Spotify link.
2. Given a user whose topic returns fewer than 10 messages for the day, when the digest job runs,
   then the system includes all available messages (<=10) and marks the digest as "partial" in metadata.

### Edge Cases
- Topic has no matching news that day → generate an audio stating "no new items" and skip Spotify publish.
...
- **FR-001**: System MUST accept a user-selected topic and language (English/Chinese) and schedule a daily digest.
- **FR-002**: System MUST aggregate up to 10 topical messages per user per day from configured news sources.
...
- **FR-005**: System MUST publish the generated audio to the user's linked Spotify account (or to a managed
  publishing account with user permission) and provide a link back to the user.
- **FR-006**: System MUST record metadata for each digest (message IDs, source attribution, language, generated audio
...
- **Digest**: user_id, date, messages[<=10], audio_url, language, status, length_seconds, publish_attempts
